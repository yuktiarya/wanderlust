...
async function connectWithFallback() {
    try {
        ...
        return { type: 'atlas', url: dbUrl };
    } catch (err) {
        console.log("Atlas connection failed:", err.message);
        const localDbUrl = "mongodb://127.0.0.1:27017/wanderlust";
        console.log(`Falling back to local MongoDB (${localDbUrl})`);
        try {
            await mongoose.connect(localDbUrl, { autoSelectFamily: false });
            console.log("Connected to local MongoDB");
            return { type: 'local', url: localDbUrl };
        } catch (localErr) {
            ...
            try {
                ...
                return { type: 'memory', url: uri, mongod };
            } catch (memErr) {
                console.log("In-memory MongoDB failed:", memErr.message);
                throw memErr;
            }
        }
    }
...
async function startServer(connInfo) {
    const store = MongoStore.create({
        mongoUrl: connInfo.url,
...
connectWithFallback().then(startServer).catch((err) => {
    console.error("Failed to connect to any MongoDB instance:", err);
    process.exit(1);
});
